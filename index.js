...
function drawGraphChart(val1, val2, canvas, multiplier) {
  var ctx = canvas.getContext("2d");
  ctx.fillStyle = "#FFFFFF";
  ctx.fillRect(0,0, canvas.clientWidth, canvas.clientHeight)
  ctx.fillStyle = "#000000";
  ctx.fillText( '--Under construction-- ',canvas.clientWidth/4 , canvas.clientHeight/2)
}  
...
function updateCharts() {
  ...
  for(let item of room1Data.reservations) { 
    ...
    res.length = new Date(item.endDate).getHours() - new Date(item.startDate).getHours()
    resArray1.push(res)
  }
  ...
  drawGraphChart(resArray1, resArray2, document.getElementById('graph'), 1)
}
...
function days_between(d1, d2) {
  ...
  let date1_ms = d1.getTime()
  let date2_ms = d2.getTime()
